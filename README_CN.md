...
1. DC-Seg 复现与表征分析  
   [中文版](reports/01_DC-Seg_Reproduction_and_Representation_Analysis_cn.pdf) · [English](reports/01_DC-Seg_Reproduction_and_Representation_Analysis_en.pdf)  
   包含训练与评估流程复现、全部 15 种缺失模态设置，以及 modality、fusion 前
   anatomical 和 fusion 后 anatomical representation 的定量分析。
2. DC-Seg 误差分解与模态贡献分析  
   [中文版](reports/02_DC-Seg_Error_Decomposition_and_Modality_Contribution_Analysis_cn.pdf) · [English](reports/02_DC-Seg_Error_Decomposition_and_Modality_Contribution_Analysis_en.pdf)  
   包含区域级与标签级误差分解、上下文平衡的模态贡献，以及代表性 failure cases。

两份报告均提供中英文版本；英文版与中文版保持相同的分析范围。
